...
class config:
    ...
    def user_mail_send(self, get):
        ...
        if self.mail.qq_smtp_send(
            get.email.strip(),
            public.lang("aaPanel Alert Test Email"),
            public.lang("aaPanel Alert Test Email"),
        ):
    ...
    def getFpmConfig(self, get):
        ...
        if not os.path.exists(file):
            return public.return_msg_gettext(
                False, "The PHP-FPM configuration file does not exist."
            )
        conf = public.readFile(file)
        if not conf:
            return public.return_msg_gettext(
                False, "Failed to read the PHP-FPM configuration file."
            )
        data = {}
  ...
  `addtime` INTEGER
    ...
    def GetSessionConf(self, get):
        filename = "/www/server/php/" + get.version + "/etc/php.ini"
        if (
            public.get_webserver() == "openlitespeed"
            and not public.get_multi_webservice_status()
        ):
        ...
        if not isinstance(phpini, str):
            return public.return_msg_gettext(
                False,
                "Failed to read PHP configuration file, it may not exist: {}".format(
                    filename
                ),
            )
    ...
    def err_collection(self, get):
        ...
        error_infos = {
            ...
            "PACK_TIME": public.readFile("/www/server/panel/config/update_time.pl")
            if os.path.exists("/www/server/panel/config/update_time.pl")
            else public.getDate(),  # 打包时间
            "TYPE": 101,
            "ERROR_ID": "{}_{}".format(
                error.split("\n")[0].strip(), get.get("uri", "")
            ),
        ...
        if not public.cache_get(pkey):
            try:
                public.run_thread(
                    public.httpPost(
                        "https://geterror.aapanel.com/bt_error/index.php", error_infos
                    )
                )
